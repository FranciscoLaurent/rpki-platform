...
class ASN(Base, TimestampMixin, TenantMixin):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False, comment="AS 名称")
    asn_type: Mapped[str] = mapped_column(
        String(50),
        nullable=False,
        default="provider",
        comment=("AS 关系类型：own/customer/provider/peer/ixp/route_server/scrubber"),
    )
    ...
    noc_phone: Mapped[str | None] = mapped_column(String(50), nullable=True, comment="NOC 联系电话")
